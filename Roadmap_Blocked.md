...
- The updater signature needs `TAURI_SIGNING_PRIVATE_KEY`, and it is not on this machine: not in the user environment, not beside the config, and `tauri.conf.json` carries only the public half. `npm run tauri build -- --bundles nsis` produces the installer and then stops on that, so a build here stages `OpenRadar_<version>_x64-setup.exe` and no `.sig`. Checked 2026-09-05, when 0.10.0 was built: 6.9 MB, correct name, no signature. Nothing else about the release path is blocked by it, but a release cannot be published from this checkout until whoever holds the key puts it in the environment.
...
- The clean Windows validation VM on the LAN was offline during the v0.1.0 build. A silent install and uninstall passed locally in a disposable directory.
...
- Confirming that uninstalling with the radar wallpaper switched on puts the reader's own picture back (`AUD-420`, 2026-09-22) needs an installed build on a desktop nobody is using. The check its acceptance names is to read `HKCU\Control Panel\Desktop\Wallpaper`, install, switch the feature on, uninstall, and read it again; run here it would change the wallpaper on the desktop in front of the owner. What is covered instead: the uninstall's rules by the wallpaper tests (ours is replaced with the note's path, a picture chosen since is left alone, a desktop that cannot be read still has ours taken down, nothing is created when nothing was taken), the hook's command line and its update guard by a test that reads `src-tauri/windows/hooks.nsh`, and the hook compiled into the installer by makensis on the same day.
