...
- [ ] AUD-453 (P2): One 1,024 pixel raster cannot both cover the window and match its resolution, and the ceiling is a maximised window at low latitude
  Measured 2026-09-08 while draining `AUD-447`, and worse than this item first said. Substituting `steps = 2^(z - T + 1)` into the bound makes the zoom cancel: coverage holds iff `wide >= W * 2.8125 / 2^T`, so it is a property of the disc's width in degrees alone, and that width goes as `1/cos(latitude)`. Against the 159-site table in `src-tauri/src/level2/registry_table.rs`, with `T` at 10: **85 of 159 sites** leave bare basemap at a 1,920 pixel window, **152 of 159** at 2,560, and **all 159** at 3,840. KDMX is among the widest CONUS discs and clears 1,920 by five per cent, which is why the fixture the coverage case uses passes. KBYX is 264 pixels short, KAMX 250, PGUA 371. A terminal radar is worse again: `src-tauri/src/tdwr.rs:249-257` clips the same box against an 89 km disc, 2.14 degrees wide, so a TDWR site is uncovered at every common window size at zoom 10 and would need `T` at 12. If the answer turns out to be a constant rather than a rule read off the disc and the live viewport, `T = 12` is the first value that covers all 159 sites and TDWR from 1,920 to 3,840; `T = 11` covers the WSR-88Ds to 2,560 and leaves TDWR short.
...
- [ ] AUD-465 (P3): A storm-report fetch aborted by the browser rather than by the workspace falls through to the second source
  ...
  Touches: `src/lib/overlays/reports.ts` (test the failure as well as the signal), `src/lib/overlays/reports.test.ts` (a case that rejects the first source with a `DOMException` named `AbortError` and asserts the second host is never fetched).
  Acceptance: an `AbortError` from the first source is rethrown and the second host receives no request; the existing fall-through on a `TypeError` still asks it; reverting the guard fails the new case.
  Complexity: S
