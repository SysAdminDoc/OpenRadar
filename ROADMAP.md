...
- [ ] AUD-120: P3. Format numbers in the language they are read in
      Why: Spanish and French both write a decimal comma, and the app writes a point everywhere a number carries a fraction. `toLocaleString(locale())` is used in the handful of places that round to a whole number, so thousands separators are already right, but the 81 `toFixed` calls are not: a tide of 3.42 ft, a depth of 1.2 m, a distance of 4.5 km and every sounding figure read as English numerals in a Spanish or French window. This was found while shipping French and deliberately left alone there, because changing it would have changed Spanish output too, which that item forbade.
      Evidence: `src/lib/units.ts` (`formatDistance`, `formatDepth`, `formatTideHeight`, `formatReportMagnitude`); `src/i18n/index.ts` (`locale`); the remaining `toFixed` callers across `src/lib` and `src/panels`
      Touches: One number formatter that takes a fraction-digit count and the current locale; the callers that print a fractional number to a reader; tests pinning the separator per language
      Acceptance: A number shown to a reader uses that language's decimal separator and grouping in every panel, legend, popup and readout; a value parsed back out of a control still round-trips, because a comma is not a decimal point to `Number`; English output is byte-identical to today's; a test fails if a new `toFixed` reaches a translated surface.
      Complexity: M
