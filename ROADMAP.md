...
- [ ] AUD-451 (P2): The failure-path gate reads a JSX attribute as an assignment and cannot follow a message through a `throw`
  ...
  Touches: `src/lib/failurePaths.test.ts` (reject an `=` immediately followed by `{` as an assignment, or bound the body at braces; treat `throw new Error(...)` whose argument is tainted as a sink). Not `onError`: draining `AUD-448` on 2026-09-08 established that the wind layer hands its `onError` the driver's own compile log, which reaches `log.warn` and never a reader, so making it a sink convicts a developer log the gate's own docblock exempts. The reader's sentence on that path is `wind.noDraw`, said by the workspace, plus a self-test that plants the JSX shape in a fixture string and asserts no conviction, and plants the `throw` shape and asserts one.
