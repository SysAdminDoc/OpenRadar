...
- [ ] AUD-359 (P2): The velocity unfolding contract fails at Tampa Bay, on a sweep nothing has checked since 2026-09-05
      State: until this is settled, `npm run check:live` exits non-zero. That is one failing assertion inside a contract which had been running nothing at all since 2026-09-05, so the run is red because it is finally asking rather than because anything got worse. A release is not blocked: `scripts/release.mjs` runs `npm run check`, rustfmt, clippy, `cargo test --lib`, the fuzzing-feature compile and the browser suite, and the live contracts remain a check somebody runs by hand.
      Why: `level2::decode::tests::unfolding_a_live_velocity_sweep_takes_the_folds_out` requires that better than one in twenty folded gates comes back to the branch it started on, at every station it samples. On 2026-09-07 five of six cleared it and KTBW did not: 615 of 16,333, which is one in 26.6. Its broken pairs barely moved either, 22,275 to 21,115 against 30 to 50 per cent improvements everywhere else, and KTBW alone also takes the aggregate assertion below its line (324,748 against 323,859, a margin of 889 out of 300,000). This is the first time the test has run since the 2026-09-05 module split broke the filter that reaches it, so nobody knows whether it is the dealiaser or the day.
      Evidence: the run of 2026-09-07 with all six stations printed: KDMX 526 of 2,274; KTLX 4,232 of 10,299; KAMX 5,442 of 9,944; KTBW 615 of 16,333; KGRR 1,757 of 4,777; KFWS 2,799 of 13,428, every one of them inventing zero readings. `src-tauri/src/level2/decode_tests.rs:191` (the per-station floor) and `:212` (the aggregate). A second run the same hour returned KTBW's numbers unchanged because the volume was cached, so there is one sample and not two.
      Touches: `src-tauri/src/level2/decode_tests.rs`, `src-tauri/src/dealias.rs`.
      Fix: Sample first, decide second. Run the contract on several separate days and keep the six stations' numbers. If KTBW is low every day, the dealiaser has a shape it does not handle and the fix is in `dealias.rs`; the sweep to look at is a coastal one where most of the folded gates sit in patches with no boundary to anything outside themselves, which is what the test's own comment says the ratio depends on. If KTBW moves with the weather and only some days are poor, the per-station floor is the wrong assertion for a quantity the test already knows is unsteady, and the aggregate it added for exactly that reason is the one to keep. Do not settle it by moving the threshold to clear whatever today happens to be.
