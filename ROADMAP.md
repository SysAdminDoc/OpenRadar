...
- [ ] AUD-433 (P3): Three readings of where a gate begins, and the one that agrees with the model is in the minority
  Why: `nexrad-model` documents `first_gate_range_km` as the range to the *centre* of the first gate (`data/moment.rs:19`). `vwp.rs:151` reads it that way. `render.rs:161` subtracts half an interval and `level2/testing.rs:182` (`gate_centre_km`) adds half an interval, both treating it as the near edge, and the two of them disagree with each other as well as with the model. Nothing is visibly wrong because a half-bin shift is invisible in a picture, which is exactly how the `AUD-379` measure ended up registering two products a whole bin apart and explaining it away in a comment: the arithmetic there had to be worked out by hand because calling the repo's own helper would have been wrong twice over. A quantity three files read three ways is a defect waiting for the next person who trusts one of them.
  Evidence: `https://docs.rs/nexrad-model` and the vendored `data/moment.rs:19` ("The range to the center of the first gate in kilometers"); `src-tauri/src/level2/render.rs:161`; `src-tauri/src/level2/testing.rs:182`; `src-tauri/src/vwp.rs:151`; `src-tauri/src/tdwr.rs:285` (`floor`, which is right for a product whose bins start at an edge); the refutation pass of 2026-09-08.
  Touches: `src-tauri/src/level2/testing.rs` (`gate_centre_km`, which should be the identity on the model's own value), `src-tauri/src/level2/render.rs`, `src-tauri/src/vwp.rs`, and a unit test that pins the convention against a fixture whose gate ranges are known, so the next reading of it has something to check itself against.
...
- [ ] AUD-437 (P3): The two terminal-radar lines are rendered by no test, at any width or in any language
  Why: `AUD-415` gave `radar.terminalLine` and `chrome.terminalRadar` a formatted distance in place of a fixed "km", and its acceptance said the pseudolocale clipping test would cover the changed keys. It does not: both need a held TDWR site before they render, and no spec holds one. What covers them today is `src/i18n/numbers.test.ts`, which asserts the template carries no literal unit after its placeholder and never renders either string. The risk the clipping sweep exists for is real here: the chrome eyebrow goes from `⟦ŦĐŴŘ · 225 ķɱ ······⟧` at twenty-two characters to `⟦ŦĐŴŘ · 140 ⟦ɱí ·⟧ ·····⟧` at twenty-five with a nested bracket, because the unit is now a translated string inside a translated string. Measured as text, not as pixels: nobody has seen it drawn.
  Evidence: A refutation pass of 2026-09-08 traced both call sites (`src/panels/RadarProductPanel.tsx:548` behind `sweep?.radar === "TDWR"`, `src/components/WorkspaceChrome.tsx:783` behind the same) and found no spec that reaches either. `e2e/language.spec.ts` opens every panel and never holds a terminal site.
