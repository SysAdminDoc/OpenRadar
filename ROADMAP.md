...
- [ ] AUD-360 (P2): The dealiaser gives up on about one sweep in eight, and nothing notices
      Why: On five of the 39 station-days recorded on 2026-09-07 the region dealiaser returned a picture as folded as it found it. It is not one station and it is not one day: the reader on those afternoons is looking at a velocity cut with the folds still in it, which is the thing the module exists to prevent, and the app says nothing about it because there is no measure of its own work anywhere in the shipped path.
      Evidence: `recording_the_days_unfolding_is_held_against` in `src-tauri/src/level2/decode_tests.rs`, run over 2026-09-01 to 2026-09-07 at 21:00 UTC. KTLX on the 4th took 26,486 broken pairs to 26,425, a fifth of a per cent, with 41 of 16,528 folded gates back on their own branch; KTLX on the 3rd 3,913 to 3,891 with 15 of 2,220; KAMX on the 4th 5,575 to 5,476 with 61 of 3,366; KFWS on the 3rd 10,439 to 10,097 with 130 of 5,661; KTBW on the 7th 19,643 to 18,879 with 429 of 13,984. The same six stations on 2026-09-01 gave 20,861 to 4,153 at KDMX and 16,768 to 4,679 at KGRR, so the method works when the sweep lets it.
      Note: the shape to suspect is in `src-tauri/src/dealias.rs`. Every patch is placed relative to the largest one and anything reaching it through no settled boundary is left where it sits, which the comment there calls the honest answer. On a fragmented sweep that is most of the picture. Py-ART's answer is a reference field, and this repo already computes one: `vad::fit_ring` and `vad::median_wind` give a wind, and `vad::Wind::along_beam` turns it into the radial velocity a gate should read, which places an orphan patch with no boundary at all.
      Note 2026-09-07, later: the placement half shipped. `dealias` now fits the ambient wind through the gates the boundary vote settled (`reference_wind`, 24 range rings through `vad::fit_ring_checked`, kept only where the profile's own `trusted()` holds) and offers every unreached patch to it, moving a patch only when three fifths of its gates agree which interval they are in. Re-recorded over the same 39 station-days: 34 better, 5 unchanged, none worse, nothing invented; KTBW on the 7th went from 429 of 13,984 folded gates back to 11,471, KGRR on the 1st from 13,183 of 50,310 to 26,670, and every day's aggregate moved the right way. Three of the five bad days (KTLX on the 3rd and 4th, KFWS on the 3rd) are among the unchanged: no ring there can be trusted, because the settled gates cover 34 to 118 radials of 720 all down one side. Relaxing the trust rule to reach them was tried and measured, and it took KTLX on the 3rd from 3,913 broken pairs to 4,227, worse than it was found, so it stays strict and the reason is in the code. What remains of the acceptance: the sweep does not yet report how much of itself it could not place. `dealias` returns the count it moved and nothing else, so a reader looking at one of those three days is still shown a folded cut with nothing on screen saying so. That is the piece left, and it is S: return the unplaced gate count beside `moved`, carry it on the sweep the way `elevation_degrees` already travels, and say it in the legend.
      Touches: `src-tauri/src/dealias.rs`, and whatever hands it the VAD wind (`src-tauri/src/level2/sweep.rs` holds the cut it would be fitted from).
      Acceptance: WHEN a velocity sweep fragments into patches that the largest one does not reach, the dealiaser SHALL place them against the VAD wind rather than leave them folded, and SHALL report how much of the sweep it could not place. The five station-days above are kept as fixtures and each shows a materially better figure than the ones recorded here; no recorded day gets worse; `invented` stays at zero everywhere.
      Complexity: M
...
- [ ] AUD-362 (P1): The reference-wind pass can break a continuous seam and can draw a correctly reported cell on the wrong branch
      Note 2026-09-07 (evening): the operational numbers, read at the source. 4DD (`dealias_fourdd.c`) brings a candidate to the nearest co-interval with `min_diff` (at most ten folds) and accepts the sounding's placement only when the residual is under `compthresh2 = 0.49 Vn`; on this item's own example (Vn 25, wind +25, cell at -10) the residual is 0.60 Vn and the cell stays. R2D2 (Feldmann et al. 2020) runs a per-region standard-deviation check against its guidance first, then moves a region only when every guidance source picks the same interval and flags it as a buffer for spatial dealiasing otherwise. HookEcho (`crates/wxdata/src/dealias.rs`) settles the whole graph with a maximum spanning tree from the largest region and then re-votes every solved region against all its neighbours for up to ten passes, and has no wind vote at all. Py-ART's region method never touches a region with no edge to the main body. Witt, Brown and Jing 2009 weight a boundary difference by a function that goes to zero as it approaches Vn, so the ambiguous case carries no vote. BowEcho's bench (`crates/bench/src/dealias_eval.rs`, Apache-2.0) scores per-gate branch correctness against a re-wrapped truth, which is the shape of the `misplaced` measure; `AUD-379` adds the RPG's own product as the external reference. Sources in RESEARCH.md of 2026-09-07 (evening).
      Category: correctness
      Where: `src-tauri/src/dealias.rs:364-403` (the `if let Some(wind) = reference_wind(...)` block shipped in `74cf9be`), `:263-279` (the boundary votes in `edges` and `adjacency`, built for every touching pair including pairs inside a loose component), `:387` (`by = ((expected - values[at]) / interval).round() as i32`), `:398` (the sixty per cent agreement); `src-tauri/src/level2/testing.rs:660-689` (`broken_pairs`), `:763-790` (`wrapped`, `rejoined`, `invented`); `src-tauri/src/dealias.rs:600` (`no_sweep_comes_out_more_discontinuous_than_it_went_in`); `CHANGELOG.md` v0.11.0, the entry beginning "The velocity cut comes back unfolded".
      Problem: Two defects, both reproduced. (1) "Unreached" means "not connected to the root's component", not "touching nothing". A loose component made of several mutually touching patches already has boundary votes in `adjacency`, but the pass discards them and votes each patch against the wind on its own, so two touching patches can be shifted by different whole intervals and a seam that was continuous becomes a fold: a sweep that went in with zero broken pairs came out with twenty. (2) A patch the radar reported correctly, isolated from the body, whose true velocity differs from the ambient wind by more than one Nyquist velocity is rounded onto the branch nearest the wind, a whole interval away from the truth. An inbound cell at -10 m/s under a 25 m/s limit came back at +40 m/s, which is the outcome the file's own comment at `:524` calls worse than the fold. On the shipped path the deviation needed is about one Nyquist velocity (28 m/s at KTBW's tight cuts), which a convective inflow reaches. The three measures the recording used are all blind to it: `broken_pairs` compares differences between neighbours, so a constant shift of a whole patch cancels and an isolated patch bordered by no-data contributes nothing either way; `wrapped`/`rejoined` count only gates that wrapped when the limit was tightened; `invented` fires only on a non-whole-interval move. So "34 better, 5 unchanged, none worse" was measured with instruments that cannot see this pass's characteristic failure, and the CHANGELOG entry and the `74cf9be` commit message overclaim ("no sweep is made worse"). A likely consequence not traced live: `src-tauri/src/vwp.rs:290` unfolds each cut and then fits VAD rings over the result, and gates the pass moved agree with the wind by construction, so a ring can now pass `MIN_FIT_GATES`, `MAX_RESIDUAL_MS` and the symmetry check on partly self-generated agreement; the barb's value is about what the boundary-placed gates said, the confidence it claims is inflated.
      Evidence: Two fresh-context refutation agents, each building the committed `dealias.rs` and `vad.rs` verbatim into a scratch crate with the reference block removed as a control. Probe F (defect 1): 360 x 200, nyquist 25, elevation 0.5; body gates 0..80 at `20 * sin(az)` (nothing folded, zero broken pairs in); island at azimuths 85..95 with patch A gates 150..160 all at -20.1 and patch B gates 160..170 at [-7, 0, 7, 10, 12, 14, 16, 18, 20, 22]. A votes +1 unanimously and moves to +29.9, B votes 0 and stays; the seam goes from 13.1 to 36.9 m/s; `big_jumps` (copied from `dealias.rs:649`) 0 -> 20; with the pass removed 0 -> 0. Probe C (defect 2): body `30 * sin(az)`, an isolated cell at -10.0 at azimuths 85..95, gates 150..170; every one of its 200 gates moved to +40; `big_jumps` read 640 -> 0 at the same moment. The second probe also measured the three recorder numbers on the probe C run: `broken_pairs` 320 -> 0, `wrapped` 10,720 and `rejoined` 10,720, `invented` 0, a perfect score while 200 gates sat 50 m/s wrong and 15 m/s outside the band the radar can report. The generator behind `no_sweep_comes_out_more_discontinuous_than_it_went_in` does build the shape (34 of its 300 seeds hold a loose component of two or more touching patches, 46 such components, the largest of four patches, and the pass fires on 50 seeds) and never the value pattern: its field is a smooth cosine plus symmetric noise, so the loose patches read alike and vote alike, and no seed puts two touching loose patches on opposite sides of the rounding line. That is why the test stayed green over 46 instances of the structure.
      Fix: Place each loose component by its own boundaries before the wind sees it: run the same heap traversal from the component's own largest patch over `adjacency`, so the relative offsets inside the component come from the boundary votes (strong evidence), then take one wind vote for the whole component (weak evidence, and only its absolute interval). Add a plausibility margin so a patch is moved only when the winning offset leaves it within a stated fraction of the interval of the wind, and leave it where it was otherwise. Return an unplaced and a wind-moved count beside `moved` from `dealias`, carry them on the sweep the way `elevation_degrees` travels, and show them in the legend; that is also the half of `AUD-360`'s acceptance still open. Add a `misplaced` measure to `measure_unfolding_bytes`: gates that never wrapped and moved anyway, asserted zero in the contract and printed by the recorder. Extend the generator of `no_sweep_comes_out_more_discontinuous_than_it_went_in` to produce loose components of two or more touching patches and an isolated correctly reported cell, and pin probes F and C as unit tests. Correct the CHANGELOG entry when the fix lands: it must not say nothing is made worse until the measure can see the case.
      Acceptance: Probe F produces zero new big jumps and probe C leaves the cell at -10 m/s, both as unit tests in `dealias.rs`; the refold contract asserts `misplaced == 0` at every station and the recorder prints it; the 39 recorded station-days re-run with no `misplaced` above zero and the aggregates no worse than the 2026-09-07 numbers written in the contract; `unfold_velocity` reports how many gates the wind placed and how many were left, and the legend says so; `cargo test --lib` and `npm run check:live` green.
      Note 2026-09-07 (evening, during the fix): one clause of that acceptance is wrong and was written without running it. `misplaced` cannot be held to zero on real volumes, and the reason is the method rather than the defect. Region dealiasing recovers each group of patches up to its own whole interval and no further, so a group nothing joins to the rest of the sweep is internally continuous on a branch nothing in the data determines, and about half of them land on the other one. Measured over 42 station-days from the archive with the input volumes held identical: 159,771 such gates with no reference pass at all (`ac6967f`), 178,265 with the pass as shipped (`74cf9be`), 243,306 with this fix. The measure is kept and printed by the recorder, because it is what made that comparison possible and it is the only number here that can see a patch moved as one piece, but the contract records it rather than asserting on it, with the figures in the comment. What holds the defect down instead is the three unit tests in `dealias.rs`, where the truth is built to order: a correctly reported cell, a seam inside an unreached group, and a fold inside one. Each was mutation-proven, failing with its own half of the fix removed.
      Confidence: Verified (two independent fresh-context probes on byte-identical copies of the committed file, each with the reference block deleted as its control; both reproduced both defects)
      Effort: M

### P2

- [ ] AUD-363 (P2): The OpenStreetMap credit line is unreadable over a light basemap under the dark theme
      Category: visual
      Where: `src/index.css:1301` (`.source-attribution`, dark: `color: rgba(229, 237, 247, 0.72)` on a transparent background), `:1321` (`:root[data-theme="light"] .source-attribution`, the only light variant), `:1341` and `:1336` (`.map-watermark` and its light rule), `:1282` and `:1297` (`.map-readout`); `:2991` (`.ambient-readout[data-over-light]`, the one element that keys on the basemap); `src/components/MapViewport.tsx:965` and `src/App.tsx:2650` (`isLightBasemap` feeds the county lane and `AmbientReadout` only); `src/lib/mapStyles.ts:196-207` (`grayscale`, `pro-light`, `roads`, `daylight`, `topography` are light).
      Problem: A reader on the dark theme who picks Roads, Daylight, Topography, Greyscale or Radar Light gets the map's credit line as 72 per cent pale ink with no backing over a light street map. The OpenStreetMap credit is the ODbL attribution the basemap licence requires, and it is the one line on the map that is not readable in that combination. The coordinate readout and the watermark keep their own dark pills and survive; the attribution has none.
      Evidence: Observed on 2026-09-07 at 1440x900 with `mapStyle: "roads"` and `document.documentElement.dataset.theme = "dark"`: `getComputedStyle(.source-attribution).color` is `rgba(229, 237, 247, 0.72)` and `backgroundColor` is `rgba(0, 0, 0, 0)`; `document.querySelectorAll("[data-over-light]").length` is 0. Screenshot in the session scratchpad (`audit-shots/audit-darktheme-lightmap.jpeg`): the credit is faint grey text over the water at the bottom right. The same view under the light theme reads `rgb(23, 32, 51)` on `rgba(248, 250, 252, 0.72)`.
      Fix: Set `data-over-light` on the stage (or the root) from `isLightBasemap(mapStyle)` in `MapStage`, beside where `AmbientReadout` already receives it, and key the light variants of `.source-attribution`, `.map-watermark` and `.map-readout` on `[data-over-light]` as well as `:root[data-theme="light"]` (the `.ambient-readout[data-over-light]` block at `:2991` is the pattern). Give `.source-attribution` the light pill background it has under the light theme whenever it is over a light map.
      Acceptance: With the theme dark and the basemap Roads, `.source-attribution` computes to the dark ink and its pill, and its contrast against the pill is at least 4.5:1; a test in `e2e/theme.spec.ts` sets the basemap to `roads` under the dark theme and reads the computed colour of the credit, the watermark and the readout; the dark basemaps are unchanged.
      Confidence: Verified
      Effort: S

- [ ] AUD-364 (P2): The tool hint card and the catch-up card share the top-centre anchor and draw over each other
      Category: ux
      Where: `src/index.css:5362-5368` (`.catch-up`: `position: absolute; top: 64px; left: 50%; transform: translateX(-50%); z-index: 6`); the tool hint is produced at `src/components/MapViewport.tsx:2496-2508` (`tool.drawHint`, `tool.startHint`, `tool.inspectHint`, `tool.sectionStartHint` through `onToolResult`), held in `src/App.tsx:281` (`toolResult`) and rendered by `src/components/WorkspaceChrome.tsx` from its `toolResult` prop (`:148`), as `<span className="tool-hud__result">` at `WorkspaceChrome.tsx:435-436` inside the tool HUD (`.tool-hud` at `src/index.css:2561-2565`: `position: absolute; z-index: 33; top: 76px; left: 50%`, twelve pixels below the catch-up card's anchor and twenty-seven z-levels above it).
      Problem: With the "While you were away" card on screen, starting Draw, Range, Inspector or Cross-section puts the tool's instruction card on top of it at the same top-centre position, so the catch-up card's title and first line are covered and the tool card sits on a card rather than on the map. Both are the app's own words and both are meant to be read; whichever one the reader wanted, the other is in the way. Reachable every morning: the catch-up card shows on launch and stays until dismissed, and a tool is the first thing a reader reaches for.
      Evidence: Observed on 2026-09-07 at 1440x900 in the light theme: after opening Cross-section with the catch-up card shown, the tool card ("CROSS-SECTION, Click one end of the slice, inside the radar's range") drew inside the catch-up card's box (catch-up rect x 479-899, y 64-218; the tool card at about x 515-870, y 78-125). Screenshots `audit-shots/audit-light-section.jpeg`, `audit-light-radarproducts.jpeg`, `audit-light-diagnostics.jpeg` and `audit-light-settings.jpeg` all show it.
      Fix: One stack for the top-centre cards. When both are present, place the tool hint below the catch-up card (a `--stack-offset` set from the catch-up's height, or a shared flex column container for the two), or dismiss the catch-up card when a tool starts, since a reader who picked up a tool has finished reading. The first keeps both readable and is the smaller change.
      Acceptance: With the catch-up card shown, starting each of the four tools shows both cards with non-intersecting bounding boxes, asserted in `e2e/catch-up.spec.ts`; with the catch-up dismissed the tool card sits where it does today.
      Confidence: Verified
      Effort: S
...
Ninth pass. Evidence in RESEARCH.md of the same date. Numbered on from `AUD-378`. Every host named below is already in `ALLOWED_HOSTS`. Nothing here outranks an open audit item of the same priority: `AUD-362` stays the first thing to drain, and `AUD-378` is the second.

### P1

- [ ] AUD-378 (P1): The radar list is a crate table that names a decommissioned New Orleans radar, misses the one that replaced it, lists State College twice and puts Pittsburgh seventeen kilometres east of itself
  Why: `sites_in_reach` and `wsr88d_only` take the WSR-88D list from `nexrad_model::meta::registry`, a compiled table in a crate whose last release was 2026-04-03, and the table is wrong today. A reader near New Orleans is offered `KLIX`, which the office no longer lists and which has nothing on the bucket, and is never offered `KHDC`, the radar at Hammond that replaced it and is operating. Eureka, Dover, Langley Hill and Fairbanks have no single-site radar at all. The nearest-first order around Pittsburgh is computed from a point seventeen kilometres off. State College appears twice. Nothing in the tree compares the table with the office's own station list, which `radar_status.rs` already fetches for a different purpose.
  Evidence: `src-tauri/src/level2/mod.rs:33` (`use nexrad_model::meta::registry;`), `src-tauri/src/level2/sites.rs:26` and `:36`; the pinned crate's `src/meta/registry.rs` (157 entries, `KLIX` at line 671, `KCCX` at lines 1081 and 1399, `KPBZ` at line 1097 with longitude -80.0183). Live on 2026-09-07: `https://api.weather.gov/radar/stations?stationType=WSR-88D` lists 159 stations; in the table and not the list: `KABC`, `KACG`, `KAIH`, `KAKC`, `KLIX`; in the list and not the table: `KBHX`, `KDOX`, `KHDC`, `KLGX`, `PAPD`, `RKJK`, `RKSG`, `RODN`; `KPBZ` at -80.21794 in the list; `api.weather.gov/radar/stations/KLIX` answers 404 and `KHDC` answers "Hammond, Operate"; `unidata-nexrad-level2` has objects under `2026/09/07/KHDC/` and none under `2026/09/07/KLIX/`. https://github.com/danielway/nexrad/issues/148 (2026-09-04, open) describes the same defects upstream; no crate release carries a fix.
  Touches: a table of the app's own in `src-tauri/src/level2/registry.rs` (id, name, state, coordinates, elevation), generated by a script from the NWS list and committed; `src-tauri/src/level2/sites.rs` reads it and falls back to the crate only for an id the table lacks; `src-tauri/src/radar_status.rs` (the live list is already fetched there, so the contract can share the parser); a live contract in `scripts/live-contracts-lib.mjs` that fails when the table and the office's WSR-88D list differ by an id or by more than 0.02 degrees, with the three overseas sites (`RKJK`, `RKSG`, `RODN`) included when the bucket carries their objects and listed as a documented exclusion otherwise; `src-tauri/src/level2/sites_tests.rs`; the picker's copy for a site the office has retired; `docs/asset-ledger.md` if the generator is documented there.
  Acceptance: WHEN a reader holds a place near New Orleans, THEN `KHDC` is the nearest site offered and `KLIX` is not offered; `sites_in_reach` lists `KCCX` once; the table's `KPBZ` longitude is -80.218; the live contract passes against the office's list on the day it lands and fails when an id is removed from the table; `cargo test --lib` and `npm run check:live` green.
  Complexity: M
...
- [ ] AUD-384 (P3): The working notes are six times the session budget
  Why: `CLAUDE.md` is 182 KB (30,355 words) and is read at the start of every session in this repository; the machine's own budget for that file is 30 KB, and the project's vault note was created on 2026-09-05 to hold exactly the lessons that have since grown the file back. Every session pays for the whole file before it reads a line of code.
  Evidence: `wc -c CLAUDE.md` on 2026-09-07 (182.2 KB; `wc -w` 30,355); the global budget table (`CLAUDE.md` 30 KB, enforced by `~/.claude/scripts/context-budget.ps1`); the vault note `Projects/OpenRadar Working Notes.md` ("offloaded from C:\repos\OpenRadar\CLAUDE.md", created 2026-09-05).
  Touches: `CLAUDE.md` (keep the current state, build commands, architecture and gotchas; move the dated Learned entries older than the current week to the vault note, keeping one line per lesson that changes what a session does), the vault note.
  Acceptance: `CLAUDE.md` under 30 KB; every moved lesson present in the vault note; the gotchas and architecture sections unchanged; `~/.claude/scripts/context-budget.ps1 -Report` shows no breach for this repository.
  Complexity: S
