...
- [ ] AUD-443 (P3): A lazy panel whose chunk does not arrive takes the whole window down
  Why: Every panel behind a `lazy` is rendered inside a `Suspense` with a `null` fallback and no error boundary of its own, so a chunk that fails to load throws past all of it to the root boundary in `main.tsx`. Measured with the RoutePanel chunk aborted: `[role="application"]` is gone, the map, the timeline and the command bar with it, and the reader is looking at the recovery screen because a panel they may never open did not download. The pattern predates the split of 2026-09-08, so the blast radius of one failure is unchanged; what changed is the number of independent ways it can happen for those five panels, from one to six. The same `null` fallback also shifts the map chrome before the panel exists: `data-panel-side` is set on click, so the zoom stack jumps 336 pixels the moment the button is pressed and the panel is not there for as long as the chunk takes. Measured at 1,100 ms with the chunk delayed, against 100 ms for an eagerly loaded panel.
  Evidence: measured 2026-09-08 by an adversarial review of `5ef138a..72d7530`, with `page.route("**/RoutePanel*", (r) => r.abort())`: `{"app":false,"panel":false,"boundary":true}` and the recovery screen's own text. `src/components/PanelSurfaces.tsx` (the `lazy` declarations and their `Suspense` wrappers); `src/main.tsx:24` (the only boundary).
  Touches: `src/components/PanelSurfaces.tsx` (a boundary and a fallback per panel rather than one `null` for all of them), `src/i18n/*` for what a panel that could not be fetched says, and an e2e case that aborts one chunk and asserts the map is still there.
  Acceptance: A chunk that fails to load leaves the map, the timeline and the command bar on screen and says so inside the panel's own frame; the fallback holds the panel's place rather than leaving the chrome shifted around nothing; the new case fails when the boundary is removed.
  Complexity: M

- [ ] AUD-444 (P3): The exported caption has no cap on how tall it can grow
  Why: `drawFrame` bounds the caption horizontally and nothing bounds it vertically. `boxHeight` is the wrapped line count times eighteen, so a long credit on a small export walks up the picture: measured with the credit for all twelve overlay adapters, 14 per cent of a 1280 by 720 still, 38 per cent at 640 by 360, and at 320 by 180 the box is 137 per cent of the height and its top edge is 78 pixels above the picture. The app's smallest window puts an export at about 700 pixels wide, where it is 26 per cent, so nothing today leaves the frame; what makes it worth fixing is that the credit grew from two names to as many as are drawn, and the height is the one dimension nothing watches.
  Evidence: measured 2026-09-08 by an adversarial review of `5ef138a..72d7530`. `src/lib/export.ts` (`drawFrame`, `boxHeight`, and `room` which bounds only the width); the new test `keeps a long credit inside the picture at every export size` checks the horizontal fit only and its smallest size is 480 by 320.
  Touches: `src/lib/export.ts` (a share of the picture the caption may not exceed, and what to do when it would: fewer credits, a smaller font, or the credit on one line with an ellipsis), and `src/lib/export.test.ts`.
  Acceptance: The caption box is inside the picture in both dimensions at every size down to 320 by 180 with the credit for every adapter drawn; the test measures the height as well as the width; planting a caption taller than the picture fails it.
  Complexity: S
