...
- `AUD-440`: whatever expresses the ceiling as ground resolution against the gate has to keep the coverage bound `sweepDetailBox` now enforces; the two are one constraint read from opposite ends. Since `AUD-453` shipped on 2026-09-09 the floor is a rule rather than a constant, so a terminal radar already declines to narrow until zoom 12 on its own: what `AUD-440` adds is the other end, stopping once a pixel is finer than the bin, and it has to leave the coverage halving in place rather than replacing it.
...
- [ ] AUD-467 (P3): The live loop's held volumes evict the ones in use, the way the historical hold did
  Why: `AUD-454` fixed recency in `historicalHeldRef` and left `heldRef` alone, so the two holds now behave differently for the same reason. The scrubber and the compare pane both read it with `get` and never re-insert, and both write with a bare `set` on a key that may already be present, which `Map` does not reorder. `trimHeld` then sheds by first arrival: a reader alternating between two volumes while the key space churns on product, tilt, palette or box loses the two they are using. This one also feeds `arrivedAt`, which the export caption reads, so an eviction of the volume in use drops its arrival time out of the written record as well as costing the fetch.
  Evidence: found by an adversarial pass over `9f57fb1..3225232` on 2026-09-09. `src/hooks/useSingleSiteRadar.ts` (the compare pane's read and the scrubber's read, both `heldRef.current.get(...)`; the two `heldRef.current.set(...)` sites; `arrivedAt`, which reads the same map). Bounds are `loopVolumes * 2` and `heldBoundRef` rather than the historical hold's flat eight, so the fix is the same shape and the numbers are not.
  Touches: `src/hooks/useSingleSiteRadar.ts` (delete before set at both write sites, and re-insert on a read that hits, matching what the historical hold now does), `src/hooks/useSingleSiteRadar.test.tsx` (a case that fills the loop hold, returns to the earliest volume, adds one more and asserts the returned-to volume is still held; and one that asserts `arrivedAt` still answers for it).
  Acceptance: the returned-to volume survives an eviction that would have shed it by arrival order; `arrivedAt` answers for it afterwards; both cases fail when the re-insertion is removed; the existing loop cases stay green.
  Complexity: S
