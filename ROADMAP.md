...
- [ ] AUD-236: The lightning watch forgets every place it has told the moment its feed goes quiet, so a storm is announced twice and the all-clear can be lost
      Category: correctness
      Where: `src/hooks/useLightning.ts` (the `refresh` catch at the `setWindow(null)`, and the returned `window: wanted ? current : null` with `current` aged out by `clock`); `src/hooks/useLightningWatch.ts` (the effect runs `lightningNear` then `saidRef.current = lightningAfter(...)` with no guard on a null window); `src/lib/lightningWatch.ts` (`lightningNear` answers `[]` for a null window; `lightningAfter` deletes every record whose id is not in `near`)
      Problem: The rule "come in, and half an hour after the last flash it is over" is held in `saidRef`, and that map is emptied whenever the watch runs with no window. A null window is not rare: the native command errors when the bucket listing fails or every file fails (`src-tauri/src/lightning.rs` around lines 259-312), the returned window is nulled by the clock when the newest file is older than the window plus five minutes (a ten-minute upstream gap in GLM publication does it), and it is null whenever the reader switches the lightning map layer off. In every one of those cases a place that was told "started" is forgotten: if flashes are still inside the radius on the next window it is told "started" again, and if the storm ended meanwhile the "quiet" notice, which is measured from `held.newest`, can never fire, so somebody told to come in is never told they can go back out. The approach watch does not have this defect: `src/hooks/useApproachWatch.ts` returns early on a null report and keeps its `told` map.
      Evidence: Traced `lightning.window` from `useLightning` through `src/App.tsx` (`useLightningWatch({ window: lightning.window, ... })`) into the effect. `lightningNear(null)` returns `[]` at `lightningWatch.ts:84`; `lightningAfter` prunes at `lightningWatch.ts:189-192`; the effect's only early return is `if (!rule.enabled)`, and `flashes` and `clock` are both deps so the run with a null window always happens. A scratch vitest against the real module reproduced both symptoms: started, null tick, then a window with flashes gives `["started"]` a second time; started, null tick, then empty windows at +2 min, +31 min and +24 h give `[]` every time, while the control without the null tick gives `["quiet"]` at +31 min. No test covers `useLightningWatch` as a hook.
      Fix: In `useLightningWatch`, when `flashes` is null skip the round entirely: neither announce nor call `lightningAfter`, so `saidRef` is untouched until a real window arrives. Keep the existing "forget a place that is no longer watched" behaviour by pruning against the enabled `places` rather than against `near` (pass the live place ids into `lightningAfter`, or prune in the hook the way `useApproachWatch` does). Consider deriving the watch's window from the feed independently of the map layer switch, since the settings copy (`lightningWatch.needsLayer`) currently makes the layer a prerequisite; at minimum the memory must not be tied to it.
      Acceptance: A hook test in `src/hooks/useLightningWatch.test.ts` (matching `useAlertWatch.test.ts` in style) that renders the hook with a window holding flashes inside the radius, asserts one "started", re-renders with `window: null`, then re-renders with the same window and asserts nothing is said; and a second case where the null is followed by an empty window 31 minutes later and "quiet" IS said. Both fail on the current code.
      Confidence: Verified
      Effort: S
