...
- [ ] AUD-432 (P3): No overlay layer's credit reaches an exported picture or its sidecar
  Why: An exported picture burns a credit into the corner and writes a sidecar beside it, and both are built from `sweepProvenance` or `timelineProvenance` only. `drawnRef` in `useExport.ts` is fed at `:289` and `:335` and by nothing else, so an export made with the warnings, the outlooks, the smoke analysis or any other overlay drawn over the radar credits the basemap and the radar and says nothing about them. The map's own attribution control and the diagnostics ledger both do credit them, which is what made this easy to miss: the credit exists and reaches a reader, just not on the one artefact that leaves the machine. `AUD-411`'s CHANGELOG entry claimed the export as one of its paths and was corrected on 2026-09-08.
  Evidence: `src/hooks/useExport.ts:271`, `:289`, `:298`, `:329`, `:335`, `:355`, `:400`; `src/lib/postcard.ts:119` (the burned-in caption reads the same `caption.attribution`); `src/lib/provenance.ts:479` (`attributionText`), `:522` (the sidecar), `:577` (the burned-in credit); `src/components/MapViewport.tsx:753` and `:2006` (the attribution control, which does read `adapter.attribution`). Traced by a refutation pass on 2026-09-08.
  Touches: `src/hooks/useExport.ts` (collect an `overlayProvenance` record for every adapter drawing at the moment of capture, the way `App.tsx:1934` already builds them for the ledger), `src/lib/provenance.ts` if the caption needs to fold several credits into one line without running off the picture, `src/lib/postcard.ts`, and the export tests.
  Acceptance: A picture exported with the warnings layer on carries the offices in its burned-in credit and in its sidecar; one exported with nothing over the radar is unchanged; the credit line stays inside the picture at every export size; `npm run check` green.
  Complexity: M


- [ ] AUD-435 (P3): The shell is wider than the window at 130 per cent below about 700 pixels, and everything on it goes off the left edge
  Why: `.app-shell` measures a scroll width of 558 against a client width of 524 at 681 device pixels and 130 per cent text, so the layout overflows horizontally. Once anything scrolls it, every absolutely positioned piece of map chrome moves with it: the toast host was measured at an x of minus 34 there. It is not a regression, it reads identically on either side of the toast work of 2026-09-08, and it is why that rule cannot keep its own box on screen at that size: a box cannot stay inside a container that does not.
  Evidence: Measured 2026-09-08 in headless Chromium, Upload panel open, at 681 device pixels and `textScale: 130`: `scrollWidth 558`, `clientWidth 524`, `shellScrollLeft` between 26.9 and 33.8 once focus moved, toast host `x = -34`. Identical before and after `57eac8c`. A refutation pass measured it independently.
  Touches: `src/index.css` (whatever sets a width the rail and the panel together exceed at that size; the rail is `--rail-width` and the panel `--panel-width`, and at 130 per cent both are scaled while the window is not), `src/hooks/useSettings.ts:205` if the narrow edge itself is in the wrong place for that scale, and a Playwright case that asserts `document.documentElement.scrollWidth` is not greater than `clientWidth` at each of the three layout widths and both text scales.
  Acceptance: The shell does not overflow horizontally at any of 1440, 1024, 900, 800, 700 and 660 device pixels at 100 and 130 per cent; the new case fails when a fixed-width piece of chrome is planted wider than the window.
  Complexity: M

- [ ] AUD-436 (P3): Nothing holds the forecast service to the units the request asked for
  Why: `guidance.ts` sends `forecastUnits()` with every request and keeps the unit token the service answered with on each reading, and the comment on that field says it is the only record of whether the service honoured the parameter. Nothing checks it. The one test that reads the field asserts a fixed `"°C"` beside a `setUnits("metric")` in its own setup, so the two are constants that happen to agree, and it only runs under `OPENRADAR_LIVE`, so `npm run check` never sees it. A reply that ignored the parameter would put every number a conversion away from the label the panel prints beside it, and `variableUnit` now takes that label from this app's own vocabulary rather than from the reply, so nothing downstream would notice either.
  Evidence: `src/lib/guidance.ts:88-97` (the field and its comment), `:288-294` (where the token is read), `:310` (`forecastUnits()` on the request); `src/lib/guidance.test.ts:135` (`describe.skip` without `OPENRADAR_LIVE`), `:162` (`expect(temperature.unit).toBe("°C")`); a refutation pass of 2026-09-08.
  Touches: `src/lib/guidance.ts` (a table from the system asked for to the token expected back, and a `log.warn` when they differ, which was written and dropped once for bundle size and belongs behind `AUD-431`), `src/lib/guidance.test.ts` (derive the expected token from `forecastUnits()` rather than writing it out, so the assertion moves when the setting does), and `scripts/live-contracts-lib.mjs` if the live check is the right home.
  Acceptance: The expected unit token is derived from `forecastUnits()` in the test rather than written as a literal; a reply carrying the other system's token is reported; `npm run check` green.
  Complexity: S

- [ ] AUD-439 (P3): A file opened from disk is the one sweep that never gets the reader's zoom
  Why: `sweepDetailBox` needs the site's own disc to measure a box against, and `disc` is learned from the one live whole-disc fetch of the station the map is on. A file from disk carries whatever site it was recorded at, which may be anywhere, so `historicalWithin` sends it nothing and it draws over its whole 460 kilometres at 449 metres a pixel however far the reader zooms. Sending it the live station's box instead was the bug this replaced: two discs that overlap in both axes clip to a sliver of the intersection. The file's first answer is its own whole disc and carries its own corners, so the disc for that site is in hand after one fetch; nothing records it.
  Evidence: `src/hooks/useSingleSiteRadar.ts` (`historicalWithin`, and `disc` set only when `within === null` in the live effect); `src/lib/level2.ts` (`fetchLocalSweep`'s `within`, always null from the app). Found by an adversarial review of `e8d3935..5e33650` on 2026-09-08 as M1, whose sliver half is fixed and whose "so it follows the zoom the same way" half is not.
  Touches: `src/hooks/useSingleSiteRadar.ts` (record a disc per station rather than one, or record it from any whole-disc answer including a historical one), and `src/hooks/useSingleSiteRadar.test.tsx`, whose "does not send one site's box to a file recorded at another" test pins the current null and would become a pin on the file's own box.
  Acceptance: A file opened from disk gets a box measured on its own site's disc, never on another's; the second fetch of that file is the boxed one and there is no third; and a file whose site never overlaps the map's site still draws.
  Complexity: S

- [ ] AUD-438 (P3): Zooms 8 and 9 are left on the coarse disc, and they are where the single-site view opens
  Why: The sweep is one 1,024 pixel raster over 460 kilometres, which is 449 metres a pixel. `sweepDetailBox` narrows it only from zoom 10, and the reason written beside `DISC_IS_ENOUGH_BELOW_ZOOM` used to be that a screen pixel is coarser than the raster below about zoom 9. That arithmetic was done on 256 pixel tiles; MapLibre's world is 512 times two to the zoom, so a screen pixel at forty degrees north is 14 metres at zoom 12 and 114 at zoom 9, and the real crossover is near zoom 7. So a reader at zoom 8 or 9 is already looking at the picture's grid rather than the radar's, and `SINGLE_SITE_MIN_ZOOM` is 8, which is where the view opens. The threshold cannot simply be lowered: `steps` is `2 ** (zoom - 10 + 1)`, which is 1 at zoom 9 and 0.5 at zoom 8, and both spell the whole disc. It needs a different exponent, and whatever replaces it has to keep the ceiling at sixteen and keep the first fetch of a session cheap.
  Evidence: Measured by an adversarial review of `e8d3935..5e33650` on 2026-09-08: at zoom 9 a screen pixel is 114 metres against a 449 metre raster and 250 metre gates, a ratio of 3.94, the same ratio as zooms 10 through 13 which do get a box. `src/lib/level2.ts` (`DISC_IS_ENOUGH_BELOW_ZOOM`, `sweepDetailBox`); `SINGLE_SITE_MIN_ZOOM = 8`. Commit `774e9d1` refers to this item as `AUD-437` by mistake; `AUD-437` is the terminal-radar clipping item and has nothing to do with it.
  Touches: `src/lib/level2.ts` (the exponent and the threshold, which are two spellings of one edge), and `src/lib/level2.test.ts`, whose halving test pins the current exponent and would have to pin the new one.
  Acceptance: A reader at zoom 8 and at zoom 9 gets a box finer than the disc; the ceiling still stops at a sixteenth; the halving test pins whatever the new progression is rather than asserting a monotone shrink; and the number of fetches to open a session at the default zoom does not go up.
  Complexity: S
