...
describe("what may import what", () => {
  ...
  it("says how many modules ask the runtime question, and is right about it", () => {
    ...
    // "forty-odd" and the tree held thirty-three, then thirty-six as the
    // snowfall layer, the grid rules and the melting layer asked it too. A
    // number in a comment is only worth writing down if something recounts
    // it, so this does.
    const graphed = graph();
    const asks = [...graphed]
      .filter(
      ...
      .map(([from]) => named(from));
    expect(asks.length).toBe(36);
    expect(readFileSync(join(ROOT, "lib/runtime.ts"), "utf8")).toContain(
      "thirty-six modules ask this question",
    );
  });
