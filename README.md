...
- **The weather on the bar, if you want it.** Off unless you turn it on. Rain, snow or fog drawn on the command bar while the station nearest your watched place is reporting it, from that station's own raw report, stopping when the report goes stale. It is the bar's background, so it can never be over the map, and it takes itself off if the window stops keeping up.
