...
A picture is the file that gets sent on, and the sidecar is the one left behind, so a PNG export carries the same document inside it as well: an `iTXt` chunk under the keyword `OpenRadar Provenance`, uncompressed, byte for byte what the sidecar holds. `pngcheck -t`, ExifTool and Pillow all read it, and anything that does not simply ignores it. A WebM, an MP4 and a GIF have nowhere to put it and are written as they were.
