...
[![Version](https://img.shields.io/badge/version-0.13.0-68d7ff)](https://github.com/SysAdminDoc/OpenRadar/releases)
...
- **MRMS** GRIB2 grids at one kilometre, including the products that usually cost money: rotation tracks over five windows from half an hour to a day, merged azimuthal shear at two heights, hail size, the severe hail index and the probability behind it, liquid density, integrated ice, echo tops, liquid held aloft, and what kind of precipitation is falling.
...
- **A wind profile** for a held site, read out of the radar's own velocity rather than from a model: barbs by height for each of the volumes the loop is showing, a hodograph under each, and the volume each column came from named beside it. A height the fit could not be vouched for is marked ND rather than drawn as calm air.
- **Smoothing**, optional, on a single site's sweep, and greyed out with the reason while an airport radar is held: a terminal radar's products arrive as a picture its own generator already drew, so there are no gates to read between. It reads between neighbouring gates in the radar's own polar space, never across a gate the radar did not read and never across the sign boundary in a velocity couplet, and the legend says the picture has been smoothed. Inspected readings and exported numbers are the gates themselves either way.
- **Echo mask**, optional and off by default, on a single site's sweep. It hides bird and insect blooms, ground clutter and wind farms by how well the radar's two polarisations agree and how much that agreement wanders from gate to gate, with a lower bar inside the melting layer and a strong core let back in when a storm stands over it on a higher cut. The legend says when it's on and when it couldn't run. An airport radar sends no correlation, so the switch is greyed there with the reason. Like smoothing it changes the picture only.
...
- **Name a storm** and the name follows it. The algorithm calls a cell A1, and somebody who has been watching it for two hours calls it something else. Type your own word in and it rides along the loop with the algorithm's own identifier still beside it, so a screenshot can still be checked against the office's products. A dozen at a time, held for the session, written down nowhere unless you put one in your record. When the algorithm stops tracking that storm the name goes with it, rather than landing on whichever cell inherits the number next.
...
- **Flash flood call** in Nearby and with every Inspect reading. The Weather Service's four-panel method reads the past hour's rain, how rare that rain is, the rain against flash flood guidance and the modelled runoff, and calls advisory, warning, considerable or catastrophic when three of the four reach the same bar. The app reads all four within three miles of the place and says the call, or says no three agree. The bars are the paper's own and you can move them in Settings to match a regional study. It's a guide, not a warning, and it says so.
...
- Storm reports and SPC mesoscale discussions.
- **The convective outlook, all of it.** The Storm Prediction Center's Day 1 through Day 8, and for Days 1 and 2 the tornado, hail and wind probabilities as well as the categorical risk, each drawn in the Center's own colours with the area where that hazard would be significant hatched over the top. Day 3 offers its categorical and its combined probability. The popup names the day and the issue and valid times, and if the hatching cannot be read the layer says the outlook is drawn without it rather than leaving a band that looks unhatched on purpose.
...
- Up to ten watched places that speak up when a warning reaches them, wherever the map is pointed, each with its own radius, severity floor, tone and quiet hours. One warning covering several of them is announced once and names them all. While a past storm is replaying, the History panel can run the same rules over it and list what each place would have been told and when, with your quiet hours applied and nothing actually sent. The lightning and storm-approach notices are replayed too while they're switched on, from the satellite's own files and the held radar site's storm tracking for those hours.
...
- **Storm history**: every Atlantic and eastern Pacific cyclone since 1851, drawn by intensity, and archive radar replay for the ones since 2003. A replay draws the warnings that were in force at the moment on screen, the convective outlook that stood over that day, and the storm reports that came in during the window, all out of the Iowa State archive, dated so nothing historical reads as live.
...
- **Lightning several ways.** GOES total lightning from the satellite, and from the national grids: cloud-to-ground flash density over one minute, five, fifteen or thirty, all on one scale so stepping between them compares like with like; the chance of a strike over the next thirty or sixty minutes, which is a forecast and says so; and the lightning jump, where a storm's flash rate has climbed faster than its own recent history, in standard deviations with the threshold the Warning Decision Training Division teaches marked on the control.
- **Reflectivity at the level where lightning starts.** The national grid sampled at -10 °C or -20 °C, wherever in the column the air is that cold today, which is what a forecaster reads for a storm about to electrify rather than for the rain under it.
...
- **Export the readings**, not only the picture. The sweep on screen writes as a CSV with a row per gate, its azimuth, range, latitude, longitude, beam height and value; a grid writes as a single-band float GeoTIFF cut to the view, which QGIS, GDAL, rasterio and ArcGIS open directly. The volume the sweep came from saves too, as the bucket published it. All three land beside a JSON file naming the source, the observed time, the units, the missing-value rule and anything done to the numbers. Colour tables and display thresholds are not applied, because a colour is a lossy account of a number. See [What the data export holds](#what-the-data-export-holds).
- **Export** the view as a picture, or the loop as a WebM, an MP4 or a GIF, with the time and the credits burned in. The video is encoded as fast as the frames can be drawn rather than recorded in real time. The MP4 is there because a WebM will not open in iMessage or a phone gallery; where the machine has no H.264 encoder the panel says so and the WebM still works. A JSON record lands beside the file naming the source of every frame that reached it, and a PNG carries the same record inside it. See [What the export record holds](#what-the-export-record-holds).
...
- **A full-screen view for a second monitor.** The map, a clock, what it is showing, and nothing you would ever click. It is meant to be left running on the screen you are not typing on, so the loop slows down once nobody has touched anything for a while and the picture says its own age, rather than asking a public service for a new one every minute all night. It can go into it on its own after however long you choose, and a warning where you watch takes it down and puts the workspace back.
...
- Place search, map-centred forecasts, shareable `openradar://` links that carry the held site, product, tilt and threshold as well as the camera.
- **Click an imported shape and it answers.** A placefile's hover text, a KML placemark's name, description and its own extended data: whatever the file put there, in the popup, with the file it came from named. Fields the app already drew the shape with are left out of it, because repeating the colour back at somebody who can see it is noise.
- **Imported shapes as a managed set**: up to eight GeoJSON, KML, KMZ or GRLevelX placefiles on the map at once, each with its own name, switch, opacity and place in the drawing order. Importing a file you already have replaces it rather than adding a second copy. All of them draw under the warnings. Placefiles keep their view thresholds, their time ranges, their icons and their georeferenced pictures; an icon sheet or a picture on a host the app is not allowed to fetch is named in the note rather than silently missing.
...
- English, Spanish, Canadian French and German, switched in Settings and applied where you are standing rather than on the next launch. Each translation is written by hand against the English catalogue, and only the one you are reading is downloaded.
- **Nearby weather, in words.** A map canvas has nothing a screen reader can read, so the same three questions get answered as text: which warnings cover a place, which storms the radar is tracking near it, and how far and which way each one is moving. It answers about the map centre or any place you watch, and a warning at a watched place is announced once as it arrives. Open it from Commands. The map itself takes the keyboard: tab to it, then the arrow keys move a cursor and read what the radar shows there, and plus and minus zoom, so nothing needs a drag.
...
```powershell
Get-FileHash OpenRadar_0.13.0_x64-setup.exe -Algorithm SHA256
```
...
Updates are a different matter. OpenRadar asks whether a newer version exists an hour after it starts and once a day after that, and all it does with the answer is change what the button in Diagnostics says. Nothing is downloaded and nothing is installed until you press that button. An update is signed with the project's own key and refused if the signature does not match, so the SmartScreen gap does not extend to what arrives afterwards.
...
```powershell
.\OpenRadar_0.13.0_x64-setup.exe /S
.\OpenRadar_0.13.0_x64-setup.exe /S /D=C:\Tools\OpenRadar
```
...
It does write a crash file when it goes down unexpectedly, so that a fault in a decoder leaves something to look at rather than an empty screen and no explanation. The file is written beside the app's own data and the newest five are kept. Diagnostics, under Settings, names the most recent one and where to find it. It is never sent anywhere: there is no uploader in the app and no address it could be sent to.
...
The same section counts what the record holds: rows, warnings, observations, places you named, and the period it covers. They are facts about a file, shown where you went to look for them, and never announced, badged or turned into a streak. You can stop the record being written at all from there, and then there are no figures rather than estimated ones.
...
A picture is the file that gets sent on, and the sidecar is the one left behind, so a PNG export carries the same document inside it as well: an `iTXt` chunk under the keyword `OpenRadar Provenance`, uncompressed, byte for byte what the sidecar holds. Pillow and ImageMagick both read it back, and anything that does not simply ignores it. A WebM, an MP4 and a GIF have nowhere to put it and are written as they were.

```json
{
  "format": "openradar-provenance",
  "formatVersion": 1,
  "application": "OpenRadar 0.13.0",
...
A **volume** is the third thing, and it is not this app's account of anything: it is the object the bucket published, saved byte for byte. An Archive II volume lands as `KDMX20260830_092159_V06.ar2v` and a terminal radar's Level III product as `ATL_TZ0_2026_08_30_23_40_12.nids`, under the bucket's own name because that is what every other tool expects to be handed. Its sidecar has a `kind` of `volume`, no `product` (it holds all of them), and an `object` naming the bucket and the key it came from. The `sha256` is the object's, so a file can be checked against the source it claims. Reopening one through Open a file draws the same sweep, for a volume; a Level III product is a different format and that panel reads Archive II only.
