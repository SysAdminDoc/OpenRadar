...
**Radar.** `mrms` decodes the national mosaic. Those files are GRIB2 with data representation template 41, which is a 16-bit PNG plus a linear scale, so the decode needs no GRIB library at all. `level2` is single-site NEXRAD: it lists the recent and historical archive buckets, opens local Archive II files, decodes volumes and draws one sweep as a Web Mercator PNG laid over the site's own extent. It is a directory rather than a file, split by what a change touches: `ramp` for the colours and the projection, `listing` for what the archive holds, `decode` for bytes to a scan, `sweep` for choosing a cut, `render` and `draw` for the picture, `sites` for which radar can see a place, `commands` for what the frontend calls, and `section` for a vertical slice. `chunks` is the same radar a few minutes earlier, assembling the pieces of a volume the radar is still sweeping. `level3` reads the products a site publishes ready-made, and `tdwr` is the FAA's airport radars, which publish only in that form. `dealias` unfolds velocity, and `vad` fits the wind a sweep is moving in so storm-relative velocity has something to subtract. `vwp` runs that same fit at a set of heights rather than once for the sweep, which is a wind profile, and refuses a height whose ring it cannot vouch for rather than drawing a barb nobody measured. `cross_section` cuts a volume along a line between two points. `gates` is where a range turns into a gate. The ICD puts `first_gate_range_km` at the centre of the first gate, the model's own reader truncates as though it were the near edge, and everything that samples a sweep goes through this one place, so the picture, the readout under the cursor, the vertical slice and the wind profile all agree about where a reading is. `radar_status` asks the NWS what each radar says about itself, which is how the picker knows a site is restarting rather than merely quiet. `crash` is the small second process that watches this one: a decoder walking off the end of a buffer takes the app down with no panic and no line in the log, and this leaves a minidump behind on the machine so the next launch has something to point at.

**Everything else in the sky.** `lightning` reads GOES flash centroids out of NetCDF-4, which is HDF5 underneath. `gfs` and `hrrr` decode the model fields the wind particles and the forecast tail are drawn from, including HRRR's near-surface smoke. `grib` holds the parts of that format all three GRIB readers agree on, so a convention like the sign-and-magnitude integers the scale factors are stored as is written down once rather than three times. `probsevere` reads the severe-probability model.

**Leaving with something.** `exports` writes the picture, the video and the GIF. `data_export` writes the readings rather than the picture, and `geotiff` is the single-band float raster half of that. `bundles` reads and writes the `.orb` replay file, whose layout is documented in full at the top of the module. `hex` writes the checksums the replay bundle, the data export and the incident packs carry, in one place, because they are verified against files readers already have on disk and a checksum that changed shape would reject all of them. `palette` holds the colour tables, and `contrast` is the colour-vision measurement they are held to; it is compiled into the tests only, because what ships is the ramps it vouches for rather than the arithmetic.

**Kept on the machine.** `incident_packs` owns durable offline basemaps, separately from the disposable cache: a bounded tile set is journaled with per-tile hashes, written to PMTiles, read back tile by tile and hashed before an atomic rename makes it selectable. `journal` is the reader's own record, one JSON row per line so a damaged line costs one entry rather than the file. `settings_backup` keeps a copy of the settings file before each write and puts it back when the live one will not parse, because that file is read as a whole and one bad byte in it loses every place, palette and pack at once.

**The desktop itself.** `tray` owns the icon, its menu and the small glance window; `glance` is what that window reads. `wallpaper` writes one PNG and points the desktop at it, remembering what was there before so it can be put back. `display` holds the screen on while the full-screen view is showing, on the main thread, because the Win32 call that does it is per thread and that thread ends with the process. `sound` reads the alert sound a reader chose, refusing anything too large or of a kind the picker does not offer. `host` answers for the WebView2 runtime's version, which updates on Microsoft's schedule rather than with the app. `window_geometry` centres the window at the size the app opens at, which is the one piece of the arrangement that lives outside the settings file: the crash screen's Reset layout calls it so a window restored onto a monitor that is no longer there can be recovered.
