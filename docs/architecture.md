...
**Leaving with something.** `exports` writes the picture, the video and the GIF. `data_export` writes the readings rather than the picture, and `geotiff` is the single-band float raster half of that. `bundles` reads and writes the `.orb` replay file, whose layout is documented in full at the top of the module. `hex` writes the checksums those three carry, in one place, because they are verified against files readers already have on disk and a checksum that changed shape would reject all of them. `palette` holds the colour tables, and `contrast` is the colour-vision measurement they are held to; it is compiled into the tests only, because what ships is the ramps it vouches for rather than the arithmetic.
